...
def test_base_columns():
    ...
    assert all(col in df.columns for col in expected_columns), "Colunas obrigatórias ausentes."


def test_carregar_base_coordenograma():
    df = carregar_base_coordenograma("data/coordenograma/coordenograma_base.csv")
    assert "Término" in df.columns, "Coluna 'Término' não foi gerada corretamente."
    assert pd.api.types.is_datetime64_any_dtype(df["Início Previsto"]), "Coluna 'Início Previsto' não está em datetime."


def test_validar_colunas_essenciais():
    df = pd.read_csv("data/coordenograma/coordenograma_base.csv")
    colunas = ["Ativo", "Ação", "Início Previsto", "Duração"]
    assert validar_colunas_essenciais(df, colunas), "Validação de colunas essenciais falhou."